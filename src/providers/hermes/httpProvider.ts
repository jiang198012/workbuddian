/**
 * Hermes HTTP 轻量版 provider：经 Hermes gateway 的 OpenAI 兼容 HTTP API 纯对话。
 * 这是 ACP 不可用（CLI 缺失/自检失败/启动失败）或配置远程 gateway 时的降级路径；
 * 路由选择见同目录 index.ts（HermesProvider 路由器）。
 *
 * 范围：POST /v1/chat/completions 流式 → text/done chunk；GET /v1/models 拉模型列表。
 * 不支持：工具块/批准卡/思考块/分叉/thoughtLevel（ACP 完整版才有）。
 */
...
export class HermesHttpProvider {
