...
Workbuddian is a **desktop-only Obsidian community plugin** (`isDesktopOnly: true`, Obsidian ≥ 1.7.2) that embeds the local **WorkBuddy / CodeBuddy CLI** as an AI chat agent inside a vault. It is independently built, referencing BuddyBridge (MIT) and Claudian (MIT). Windows + macOS only; Linux is unsupported. Logs are prefixed `[BB]` (BuddyBridge heritage).
