...
### 改进
- **节奏与内容**：移除长时间静止画面，保留旧版高信息密度主流程，并接入新版笔记地图与斜杠命令演示。
- **README 媒体**：GIF 更新为 45.5 秒、936×585、10fps，满足原版 29.5 秒的 1.5 倍长度要求。
...
### 修复
- **停止生成即时生效**：按会话取消流并立即结束 UI 状态，避免迟到的 ACP 响应重新触发生成。
- **ACP 启动容错**：放宽 ACP 握手与 Hermes 自检超时，降低冷启动误降级概率。
- **主编辑区大面板**：命令严格复用或创建主编辑区 Workbuddian 面板，不再误用侧栏 leaf。
- **模型状态可见**：模型按钮显示当前模型，并同步无障碍标签、标题和模型标识。
- **删除会话同步**：删除确认改为可访问对话框，并同步刷新双面板标签、搜索结果和当前会话。

### 验证
- `npm run build` 通过。
- 44 个测试套件、506 个测试通过。
- 在 `demo-vault` 复核模型标签、主编辑区大面板和删除会话双面板同步；真实 ACP 工具/批准链路仍按报告标记为环境阻塞。

## v2.6.3 — 2026-08-31

**会话工作区与回复写回**：补齐会话级配置/草稿恢复，并支持将助手回复插入当前笔记或保存为新笔记。

### 新增
- **会话工作区**：模型、授权模式、推理强度和常驻指令支持按会话覆盖；可分别控制 Vault 上下文与当前笔记链接注入。
- **草稿恢复**：输入内容与附件按会话持久化，切换、重启或恢复会话后可继续编辑；分叉会话不会复制原草稿。
- **助手回复写回**：助手回复新增“插入回复”和“保存回复为笔记”操作，保存笔记带来源、会话和创建时间元数据。

### 测试
- 44 个测试套件、505 个测试通过；在 `demo-vault` 完成草稿隔离/恢复及两种回复写回验收。
...
### 新增
- **批量导出所有会话**：命令面板「导出所有会话为笔记」,非空会话合并成一个 Markdown 文件(带分隔线,文件名带日期)。
- **代码块插入/保存到笔记**：AI 回复的代码块 hover 出三按钮——复制 / **插入到当前笔记**(追加到当前打开的笔记末尾)/ **保存为新笔记**。

### 改进
- **斜杠命令清理**：删掉与工具栏/命令面板重复的 `/model` `/permissions` `/export`;CLI 透传命令(`/compact` `/context` `/cost` `/status`)补全里加 `(CLI)` 标注,区分本地/透传/模板命令。
...
### 新增
- **消息级操作**：消息行 hover 出操作菜单——user 消息「编辑并重发」(截断该条及之后,原文填入输入框待编辑),assistant 消息「重新生成」(截到该条前,基于上文重答)。
- **会话模板**：新建按钮旁「用模板新建」,选预设场景(写作/翻译/代码审查/笔记整理)自动带常驻指令 + 预填开场白。
- **浮动内联编辑**：选中文字 → 右键「Workbuddian编辑」/ Cmd+Shift+E → 选区上方浮动工具条,就地改不弹窗。纸飞机发送按钮,Esc/点击外部关闭;diff 就地预览;右键触发保留选区并聚焦。

### 修复
- 最近一条回复缺少复制按钮(流式结束补渲染)。
- 完全访问模式仍弹批准卡(bypass 自动选 allow_always 响应)。
...
- 描述超 250 字符限制（255）且含 "Obsidian" 字样（市场语境冗余）。
- 重写为 189 字符、无 "Obsidian" 的合规描述。

## v2.2.1 — 2026-08-09

修复：最近一条回复下方缺少复制按钮。

- 根因：流式中的消息行以 isWaiting=true 创建（不渲染复制按钮），流式结束后跳过整体重建（避免销毁工具行/diff/批准卡），按钮永不补上。
- 修复：流式结束后给最近消息行补渲染复制按钮（已存在则不重复）。

## v2.2.0 — 2026-08-09

大版本：会话管理与 MCP 能力全面升级 + 中文体验深耕 + 鲁棒性测试基建。644 项单测全绿。

### 会话管理（dual-pane + 组织）
- **dual-pane 常驻会话管理器**：主编辑区大面板左侧新增常驻会话列表栏（新建 + 搜索 + 竖向列表）；侧栏窄面板保持顶部标签不挤占。
- **会话置顶/星标**：右键「置顶/取消置顶」，置顶会话排最前 + 📌 标识。
- **会话搜索**：标签栏搜索框实时过滤（标题 + 消息正文），命中高亮；`/resume` 选择器加搜索过滤 + 无结果提示。
- **删除会话确认**：Notice 二次确认（点「确认删除」才真删），防误删。
- **会话导出增强**：导出 Markdown 带元数据行 + 时间戳 + 附件标注 + 错误标记。

### MCP 与模型
- **context-saving MCP**：`@mcp/xxx` 激活才注入会话，未引用不占上下文（省 token）。
- **国内模型 UI**：模型菜单显示中文名（DeepSeek 深度求索 / GLM 智谱 / Kimi 月之暗面 / MiniMax 稀宇），国内模型优先排序。
- **CodeBuddy 插件管理**：设置页扫描 `~/.codebuddy/plugins` 按市场列出插件，启停/更新经 CLI。

### 效率工具
- **模板 prompt**：`/translate` `/summarize` `/rewrite` `/polish` `/review` `/explain` 六个常用模板一键填入。
- **@stats vault 统计**：消息含 `@stats` 注入 vault 概况（文件/笔记/标签/示例）。
- **命令面板增强**：新建对话 / 编辑常驻指令 / 打开设置 / 导出会话 / 搜索会话五个新命令。
- **上下文用量预警条**：用量 ≥80% 显示「上下文已用 NN%」+ 一键压缩/新建。

### 工程与测试
- **e2e 测试环境修复**：start-obsidian-debug.sh 临时 pin demo-vault 为唯一 open（避免 Obsidian 恢复正式 vault 导致测试加载旧插件）。
- **拟人化测试脚本**（`scripts/e2e/humanlike-test.mjs`）：computer-use 风格驱动真实 Obsidian，覆盖 dual-pane/置顶/搜索/删除确认/模板/@stats/模型中文名（11 项）。
- **鲁棒性测试方案**（`docs/release-robustness-test-plan.md`）：45 项用户视角测试矩阵 + 发布门禁。
- 644 项单测全绿（累计新增 27 项）。
